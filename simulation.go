...
func simulationTab() fyne.CanvasObject {
	...
	for i := 500; i < 600; i++ {
		log.Println(ThreeDView.Point3D{X: 0, Y: float64(i), Z: 0}, " -> ", camera.Project(ThreeDView.Point3D{X: 0, Y: float64(i), Z: 0}))
	}
	for i := -100; i < 100; i++ {
		log.Println(ThreeDView.Point3D{X: 0, Y: 0, Z: float64(i)}, " -> ", camera.Project(ThreeDView.Point3D{X: 0, Y: 0, Z: float64(i)}))
	}
